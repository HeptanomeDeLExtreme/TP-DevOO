...
public class VueTextuelle extends JScrollPane implements Observer {
	...
	if (tournee != null && tournee.getCoutTotal() != -1) {
		...
		for (Itineraire itineraire : listeItineraire) {
		    ...
		    html += "Partir de x = " + interDepart.getX() + " y = "
			    + interDepart.getY() + " id = "
			    + interDepart.getId() + "<br>";
			...
			if (courant.getNomDeRue().equals(
				precedent.getNomDeRue())) {
			    longueur += courant.getLongueur();
			} else {
			    html += "Prendre " + precedent.getNomDeRue()
				    + " sur " + longueur + "m.<br>";
			    longueur = courant.getLongueur();
			}
		    }
		    if (courant != null) {
			html += "Prendre " + courant.getNomDeRue() + " sur "
				+ longueur + "m.<br>";
		    }
		    if (listeTroncon.size() == 1) {
			html += "Prendre " + listeTroncon.get(0).getNomDeRue()
				+ " sur " + longueur + "m.<br>";
		    }
		    ...
		    html += "Arriver à  x = " + interArrivee.getX() + " y = "
			    + interArrivee.getY() + " id = "
			    + interArrivee.getId() + "<br>";
		    if (arrivee.getHeureArrivee() != null) {
			html += "Heure d'arivée estimée : "
				+ arrivee.getHeureArrivee() + "<br>";
		    }
		    if (arrivee.getHeureLivraison() != null) {
			html += "Heure de livraison estimée : "
				+ arrivee.getHeureLivraison() + "<br>";
		    }
		    if (arrivee != entrepot) {
			if (!arrivee.getEstDansFenetre()) {
			    html += "<FONT color=\"red\">" + "RETARD"
				    + "</FONT><br>";
			}
		    }
			...
			html += "Fenetre : "
				+ arrivee.getFenetre().getHeureDebut() + " "
				+ arrivee.getFenetre().getHeureFin()
				+ "<br><br>";
		    }
		}
		html += "Temps de tournée : "
			+ new Horaire(tournee.getCoutTotal()) + "<br>";
