...
public class Tournee extends Observable {
     ...
     *            Map associant les intersections aux sommets du GraphePondere
     * @param coutTotal
     *            Cout total de la tournee
     * @param livraisonsEnOrdre
     *            Livraisons dans l'ordre de la tournee
     * @param itinerairesEnOrdre
     *            Itineraires dans l'ordre de la tournee
     */
    public void charge(Map<Integer, Intersection> correspondancePlan,
	    DemandeDeLivraison ddl, Livraison entrepot, int coutTotal,
	    List<Livraison> livraisonsEnOrdre,
	    List<Itineraire> itinerairesEnOrdre) {
	...
	for (Itineraire itineraire : itinerairesEnOrdre) {
	    ...
	    List<Troncon> listeTroncon = depart.rechercherTroncons(
		    correspondancePlan, arrivee);
	}
    ...
    /**
     * Ajoute une Livraison dans la tournee avant une autre livraison. La
     * livraison cree sera dans la meme fenetre que la livraison cible.
     * 
     * @param livraisonSuivante
     *            Livraison avant laquelle on veut creer la livraison.
     * @param lintersectionCible
     *            Intersection sur laquelle on veut placer la livraison.
     * @return Livraison cree
     */
    public Livraison ajouteLivraison(Livraison livraisonSuivante,
	    Intersection intersectionCible) {
	...
	Livraison livraison = new Livraison(id_max, philippe,
		intersectionCible, livraisonSuivante.getFenetre());
	livraison.calculerPlusCourtsChemins(graphePondere);

	if (livraisonSuivante.equals(livraisonsEnOrdre.get(livraisonsEnOrdre
		.size() - 1))) {
	    ...
	    livraisonsEnOrdre.add(livraisonsEnOrdre.indexOf(livraisonSuivante),
		    livraison);
	}
	...
	if (livraisonSuivante.equals(entrepot)) {
	    livraisonPrecedente = livraisonsEnOrdre.get(livraisonsEnOrdre
		    .size() - 2);
	} else {
	    livraisonPrecedente = livraisonsEnOrdre.get(livraisonsEnOrdre
		    .indexOf(livraison) - 1);
	...
	int coutLivraisonToSuivant = livraison.rechercherCout(
		graphePondere.getMapCorrespondance(), livraisonSuivante);
	int coutPrecedentToLivraison = livraisonPrecedente.rechercherCout(
		graphePondere.getMapCorrespondance(), livraison);

	List<Troncon> tronconsLivraisonToSuivant = livraison
		.rechercherTroncons(graphePondere.getMapCorrespondance(),
			livraisonSuivante);
	List<Troncon> tronconsPrecedentToLivraison = livraisonPrecedente
		.rechercherTroncons(graphePondere.getMapCorrespondance(),
			livraison);

	Itineraire itLivraisonToSuivant = new Itineraire(
		coutLivraisonToSuivant, tronconsLivraisonToSuivant, livraison,
		livraisonSuivante);

	Itineraire itPrecedentToLivraison = new Itineraire(
		coutPrecedentToLivraison, tronconsPrecedentToLivraison,
	...
	for (Itineraire it : this.itineraires) {
		...
		itineraires
			.add(itineraires.indexOf(it), itPrecedentToLivraison);
		itineraires.add(itineraires.indexOf(it) + 1,
			itLivraisonToSuivant);
		itineraires.remove(it);
		break;
	    }
	}

	coutTotal = coutTotal - coutASoustraire + coutLivraisonToSuivant
		+ coutPrecedentToLivraison;
	...
	charge(graphePondere.getMapCorrespondance(), demandeDeLivraison,
		entrepot, coutTotal, livraisonsEnOrdre, itineraires);
	return livraison;
    }
	...
	Livraison livraisonPrecedente = livraisonsEnOrdre.get(livraisonsEnOrdre
		.indexOf(livraison) - 1);
	Livraison livraisonSuivante = livraisonsEnOrdre.get(livraisonsEnOrdre
		.indexOf(livraison) + 1);
	...
	// Suppression des itinéraires "Precedent à Livraison" et
	// "Livraison à Suivant"
	// Insertion de l'itineraire "Precedent à suivant"
	itiLivraisonToSuivant = itineraires.get(itineraires
		.indexOf(itiPrecedentToLivraison) + 1);
	coutLivraisonToSuivant = itiLivraisonToSuivant.getCout();

	coutPrecedentToSuivant = livraisonPrecedente.rechercherCout(
		graphePondere.getMapCorrespondance(), livraisonSuivante);
	List<Troncon> tronconsPrecedentToSuivant = livraisonPrecedente
		.rechercherTroncons(graphePondere.getMapCorrespondance(),
			livraisonSuivante);
	Itineraire itiPrecedentToSuivant = new Itineraire(
		coutPrecedentToSuivant, tronconsPrecedentToSuivant,
		livraisonPrecedente, livraisonSuivante);

	itineraires.add(itineraires.indexOf(itiPrecedentToLivraison),
		itiPrecedentToSuivant);

	itineraires.remove(itineraires.get(itineraires
		.indexOf(itiPrecedentToLivraison) + 1));
	itineraires.remove(itiPrecedentToLivraison);

	coutTotal = coutTotal - coutLivraisonToSuivant
		- coutPrecedentToLivraison + coutPrecedentToSuivant;
	...
	if (livraison2
		.equals(livraisonsEnOrdre.get(livraisonsEnOrdre.size() - 2))) {
	    Inversion(livraisonsEnOrdre.get(livraisonsEnOrdre.size() - 3),
		    livraisonsEnOrdre.get(livraisonsEnOrdre.size() - 2));
	    Inversion(livraison1,
		    livraisonsEnOrdre.get(livraisonsEnOrdre.size() - 3));
	    Inversion(livraisonsEnOrdre.get(livraisonsEnOrdre.size() - 3),
		    livraisonsEnOrdre.get(livraisonsEnOrdre.size() - 2));
	}

	else if (livraison1.equals(livraisonsEnOrdre.get(livraisonsEnOrdre
		.size() - 2))) {
	    Inversion(livraisonsEnOrdre.get(livraisonsEnOrdre.size() - 3),
		    livraisonsEnOrdre.get(livraisonsEnOrdre.size() - 2));
	    Inversion(livraison2,
		    livraisonsEnOrdre.get(livraisonsEnOrdre.size() - 3));
	    Inversion(livraisonsEnOrdre.get(livraisonsEnOrdre.size() - 3),
		    livraisonsEnOrdre.get(livraisonsEnOrdre.size() - 2));
	}
	...
	Livraison livraisonSuivante1 = livraisonsEnOrdre.get(livraisonsEnOrdre
		.indexOf(livraison1) + 1);
	Livraison livraisonSuivante2 = livraisonsEnOrdre.get(livraisonsEnOrdre
		.indexOf(livraison2) + 1);
	...
	if (livraisonSuivante1.equals(livraison2)) {
	    ...
	    if (livraisonSuivante2.equals(livraisonsEnOrdre
		    .get(livraisonsEnOrdre.size() - 1))) {
		...
		ajouteLivraison(
			livraisonsEnOrdre.get(livraisonsEnOrdre.size() - 2),
			livraison2.getAdresse()).setFenetre(fenetre2);
		...
		ajouteLivraison(livraisonSuivante2, livraison1.getAdresse())
			.setFenetre(fenetre1);
		ajouteLivraison(
			livraisonsEnOrdre.get(livraisonsEnOrdre
				.indexOf(livraisonSuivante2) - 1),
			livraison2.getAdresse()).setFenetre(fenetre2);
	    }
	}
	    ...
	    // On veut intervertir les deux dernieres
	    if (livraisonSuivante1.equals(livraisonsEnOrdre
		    .get(livraisonsEnOrdre.size() - 1))) {
		supprimeLivraison(livraison2);
		...
		ajouteLivraison(
			livraisonsEnOrdre.get(livraisonsEnOrdre.size() - 2),
			livraison1.getAdresse()).setFenetre(fenetre1);
	    } else {
		supprimeLivraison(livraison1);
		supprimeLivraison(livraison2);
