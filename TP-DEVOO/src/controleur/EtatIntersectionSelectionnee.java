...
public class EtatIntersectionSelectionnee extends EtatDefaut {
	...
	Livraison liv = ddl.cherche(p, fenetre.getEchelleX(),
		fenetre.getEchelleY());
	...
	if (liv == null) {
	    ...
	    Controleur
		    .setEtatCourant(Controleur.etatLivraisonPrecedenteSelectionnee);
	    Controleur.etatLivraisonPrecedenteSelectionnee
		    .init(this.inter, liv);
	    fenetre.afficheMessage("Intersection sélectionnée : "
		    + this.inter.getId()
		    + " - Livraison suivante sélectionnée : "
		    + liv.getAdresse().getId());
	} else {
	    Controleur.setEtatCourant(Controleur.etatTourneeCalculee);
	    fenetre.afficheMessage("Sélection vide");
	}
