...
public class CommandeSuprime implements Commande {
	...
	List<Livraison> livEnOrdre = this.modele.getTournee()
		.getLivraisonsEnOrdre();
	this.livraisonSuivante = livEnOrdre.get(livEnOrdre
		.indexOf(this.livraisonAjoutee) + 1);
	...
	if (livraison.equals(livEnOrdre.get(livEnOrdre.size() - 2))) {
	    int response = JOptionPane
		    .showConfirmDialog(
			    null,
			    "Si vous supprimez la derniere livraison, des erreurs peuvent subvenir.",
	...
	if (this.commandeValide) {
	    ...
	    this.modele.getDemandeDeLivraison().majHorairesDesLivraisons(
		    this.modele.getTournee().getItineraires());
	...
	if (this.commandeValide) {
		...
		this.livraisonAjoutee = this.modele.getTournee()
			.ajouteLivraison(this.livraisonSuivante,
		...
		this.modele.getTournee().modifierTournee(derniere,
			this.livraisonAjoutee);

		System.out.println("OPHELIE8bis : " + this.livraisonAjoutee);
		this.modele
			.getTournee()
			.getLivraisonsEnOrdre()
			.get(this.modele.getTournee().getLivraisonsEnOrdre()
				.size() - 2).setFenetre(fenetreTemp);
