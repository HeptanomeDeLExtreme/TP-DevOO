...
public class DemandeDeLivraisonTest {
    ...
    @Test
    public void chargerLivraisonTest() throws ParserConfigurationException,
	    SAXException, IOException, ExceptionXML {
	Plan plan = new Plan();
	DeserialiseurPlanXML.charger(plan, new File(
		"src/tests/files/planTestOC.xml"));
	Tournee tournee = new Tournee();
	DemandeDeLivraison ddl = new DemandeDeLivraison(tournee);
	DeserialiseurDemandeDeLivraisonXML.charger(ddl, plan, new File(
		"src/tests/files/livraisonsTestOC1.xml"));

	FenetreTemporelle fenetre1 = new FenetreTemporelle(
		new Horaire(8, 30, 0), new Horaire(8, 31, 0));
	FenetreTemporelle fenetre2 = new FenetreTemporelle(
		new Horaire(8, 31, 0), new Horaire(8, 32, 0));
	FenetreTemporelle fenetre3 = new FenetreTemporelle(
		new Horaire(10, 0, 0), new Horaire(10, 30, 0));
	...
	Livraison livraison1 = new Livraison(1, new Client(631),
		plan.recupererIntersectionParId(1), fenetre1);
	Livraison livraison2 = new Livraison(5, new Client(309),
		plan.recupererIntersectionParId(5), fenetre2);
	Livraison livraison3 = new Livraison(3, new Client(396),
		plan.recupererIntersectionParId(3), fenetre2);
	Livraison livraison4 = new Livraison(15, new Client(309),
		plan.recupererIntersectionParId(0), fenetre3);
	Livraison livraison5 = new Livraison(4, new Client(396),
		plan.recupererIntersectionParId(4), fenetre3);
	...
	Plan plan1 = new Plan();
	DeserialiseurPlanXML.charger(plan1, new File(
		"src/tests/files/planTestOC.xml"));
	...
	try {
	    DeserialiseurDemandeDeLivraisonXML.charger(ddl1, plan1, new File(
	...
	assertEquals(listeFenetresAttenduesVide.size(),
		listeFenetreObtenue.size());
    }

    @Test
    public void calculerTourneeTest() throws ParserConfigurationException,
	    SAXException, IOException, ExceptionXML {
	Plan plan = new Plan();
	DeserialiseurPlanXML.charger(plan, new File(
		"src/tests/files/planTestOC.xml"));
	...
	FenetreTemporelle fenetre2 = new FenetreTemporelle(
		new Horaire(10, 0, 0), new Horaire(11, 30, 0));
	List<FenetreTemporelle> listeFenetresAttendues = new ArrayList<FenetreTemporelle>();
	listeFenetresAttendues.add(fenetre1);
	listeFenetresAttendues.add(fenetre2);
	Livraison livraison1 = new Livraison(1, new Client(631),
		plan.recupererIntersectionParId(1), fenetre1);
	Livraison livraison2 = new Livraison(3, new Client(309),
		plan.recupererIntersectionParId(3), fenetre2);
	Livraison livraison3 = new Livraison(5, new Client(396),
		plan.recupererIntersectionParId(5), fenetre2);
	Set<Livraison> setLiv1 = new HashSet<Livraison>();
	setLiv1.add(livraison1);
	Set<Livraison> setLiv2 = new HashSet<Livraison>();
	...
	List<Livraison> listeLivraisonAttendues = ddl.getTournee()
		.getLivraisonsEnOrdre();

	List<Livraison> listeLivraisonsObtenu = ddl.getTournee()
		.getLivraisonsEnOrdre();
	assertEquals(listeLivraisonAttendues, listeLivraisonsObtenu);

	List<Itineraire> listeItineraireObtenu = ddl.getTournee()
		.getItineraires();
	...
	for (Itineraire iti : listeItineraireObtenu) {
	    ...
	    if ((iti.getArrivee().getAdresse().getId()).equals(resultat)) {
	    } else {
		resultatBool = false;
	    }
	    compteur++;
	...
	assertEquals(3140, tournee.getCoutTotal());
	...
	assertTrue(liv1Obtenu.getHeureLivraison().equals(
		liv1.getHeureLivraison()));
    }
    ...
    @Test
    public void chercheTest() throws ParserConfigurationException,
	    SAXException, IOException, ExceptionXML {
	Plan plan = new Plan();
	DeserialiseurPlanXML.charger(plan, new File(
		"src/tests/files/planTestOC.xml"));
	...
	Livraison resultat = ddl.cherche(new Point(plan
		.recupererIntersectionParId(1).getX(), plan
		.recupererIntersectionParId(1).getY()), 1, 1);
