...
public class DemandeDeLivraison extends Observable {
	...
	for (FenetreTemporelle fenetre : this.fenetres) {
	    ...
	    List<Livraison> listLivraisonsFenetre = new ArrayList<Livraison>(
		    livraisonsFenetres);
    ...
    public Intersection getKeyByValue(Map<Intersection, Integer> map,
	    Integer value) {
	...
	// Calcul des plus courts chemins a partir d'un livraison sur tout le
	// plan
	calculDesPlusCourtsChemins(plan, graphePondere);

	// Creation des correspondances entre un sommet (Integer) et une
	// livraison
	Map<Integer, Livraison> mapLivraisons = correspondanceLivraisons();

	// Generation des arcs du graphe de livraisons
	int couts[][] = genererTableauArcs(
		graphePondere.getMapCorrespondance(), mapLivraisons);
	...
	itinerairesEnOrdre = recupererItinerairesEnOrdre(
		graphePondere.getMapCorrespondance(), livraisonsEnOrdre,
		mapLivraisons, couts);

	// Mise à jour des données concernant le moment où la livraison
	// s'effectue
	...
	Horaire heureDepartEntrepotDebutTournee = livraisonsEnOrdre.get(1)
		.getFenetre().getHeureDebut();
	Horaire heureArriveeEntrepotFinTournee = livraisonsEnOrdre.getLast()
		.getHeureArrivee();
	Horaire tempsTournee = heureArriveeEntrepotFinTournee
		.soustraireHoraire(heureDepartEntrepotDebutTournee);

	// Créer la tournée
	this.tournee.charge(graphePondere.getMapCorrespondance(), this,
		entrepot, coutTotalSolution, livraisonsEnOrdre,
		itinerairesEnOrdre);
	...
	Horaire heureDepartEntrepotDebutTournee = livraisonsEnOrdre.get(1)
		.getFenetre().getHeureDebut();
	Horaire heureArriveeEntrepotFinTournee = livraisonsEnOrdre.get(
		livraisonsEnOrdre.size() - 1).getHeureArrivee();
	...
	for (Itineraire unItineraire : itinerairesEnOrdre) {
	    ...
	    if (compteur == 0) {
		// 1er itinéraire : contient comme livraison de départ
		// l'entrepôt.
		...
		// Dernier itinéraire : contient comme livraison d'arrivée
		// l'entrepôt
	    ...
	    if (compteur == 0) {
		heureArrivee = livraisonDepart.getHeureArrivee()
			.additionnerHoraire(horaireCoutTotal);
	    } else {
		heureArrivee = livraisonDepart.getHeureLivraison()
			.additionnerHoraire(horaireCoutTotal);
	    }
		...
		estDansFenetre = heureLivraison.isInFenetreTemporelle(
			fenetre.getHeureDebut(), fenetre.getHeureFin());
	...
	Horaire resultat = new Horaire(resultatAdd.getHeure(),
		resultatAdd.getMinute(), resultatAdd.getSeconde());
	return resultat;
    }
     ...
     * @return
     */
    private boolean isHeureDansFenetreTemporelle(Horaire heure,
	    Horaire horaireDebut, Horaire horaireArrivee) {
	boolean resultat = heure.isInFenetreTemporelle(horaireDebut,
		horaireArrivee);
	return resultat;
    }
     ...
     * livraisons) et un plan. Le resultat du calcul (tableau pi et d) est
     * stocke directement dans l'objet livraison.
     * 
     * @param plan
	...
	// Recherche des plus courts chemins pour toutes les livraisons du
	// fichier
	// XML
    ...
    /**
     * Cree un tableau de correspondance entre une livraison et un numero de
     * sommet (Integer). Cette correspondance permet de connaitre la relation
     * entre les sommets du GrapheLivraisons et les livraisons.
     * 
    ...
    /**
     * Permet de generer un double tableau de couts representant les couts pour
     * chaque arc du GrapheLivraison. Le double tableau est base sur des Integer
     * representant des livraisons : cout[livraisonDepart][livraisonArrivee].
     * 
     * @param map
     *            map de correspondance entre un sommet et une livraison.
     * 
     * @return tableauArcs Double tableau de couts (int) qui represente un cout
     *         entre une livraison de depart et une livraison d'arrivee.
     * 
     * @see correspondanceLivraisons
     */
    protected int[][] genererTableauArcs(Map<Integer, Intersection> map2,
	    Map<Integer, Livraison> map) {
	...
	for (Livraison livraisonArrivee : livraisonsFenetre) {
	    ...
	    tableauArcs[numSommetEntrepot][numSommetArrive] = entrepot
		    .rechercherCout(map2, livraisonArrivee);
	}
	...
	for (int i = 0; i < fenetres.size() - 1; i++) {
	    ...
	    Set<Livraison> livraisonsFActuelle = fenetreActuelle
		    .getLivraisons();
	    Set<Livraison> livraisonsFSuivante = fenetreSuivante
		    .getLivraisons();
		...
		for (Livraison livraisonDestActuelle : livraisonsFActuelle) {
		    if (livraisonDestActuelle.getId() != livraisonSourceActuelle
			    .getId()) {
			Integer integerSource = getKeyByValue(map,
				livraisonSourceActuelle);
			Integer integerDest = getKeyByValue(map,
				livraisonDestActuelle);
			tableauArcs[integerSource][integerDest] = livraisonSourceActuelle
				.rechercherCout(map2, livraisonDestActuelle);
		    }
		}
		...
		for (Livraison livraisonDestSuivante : livraisonsFSuivante) {
		    Integer integerSource = getKeyByValue(map,
			    livraisonSourceActuelle);
		    Integer integerDest = getKeyByValue(map,
			    livraisonDestSuivante);
		    tableauArcs[integerSource][integerDest] = livraisonSourceActuelle
			    .rechercherCout(map2, livraisonDestSuivante);
		}
	    }
	}

	// Traitement de la derniere fenetre, pour chaque livraison de la
	// fenetre
	...
	for (Livraison livraisonSourceActuelle : livraisonsFActuelle) {
		...
		tableauArcs[integerSource][integerEntrepot] = livraisonSourceActuelle
			.rechercherCout(map2, entrepot);
	    }
	}
     ...
     * 
     * @param mapLivraisons
     *            Map de correspondance permettant de retrouver les livraisons a
     *            partir du numero de sommet.
     * 
     * @return Liste de livraisons selon l'ordre trouve par TSP.
     */
    private LinkedList<Livraison> recupererLivraisonsEnOrdre(
	    GrapheLivraisons graphe, Map<Integer, Livraison> mapLivraisons) {
	LinkedList<Livraison> livraisonsEnOrdre = new LinkedList<Livraison>();
	...
	livraisonsEnOrdre.getLast().setFenetre(
		fenetres.get(fenetres.size() - 1));
     ...
     * 
     * @param mapLivraisons
     *            La map de correspondance permettant de connaitre une livraison
     *            a partir d'un sommet.
     * 
     * @return Liste d'itinéraires en fonction de la liste de livraisons.
     */
    private List<Itineraire> recupererItinerairesEnOrdre(
	    Map<Integer, Intersection> map,
	    LinkedList<Livraison> listeLivraisons,
	...
	for (int i = 0; i < listeLivraisons.size() - 1; i++) {
	    ...
	    List<Troncon> troncons = livraisonActuelle.rechercherTroncons(map,
		    livraisonSuivante);

	    Itineraire itineraire = new Itineraire(coutItineraire, troncons,
		    livraisonActuelle, livraisonSuivante);
    ...
    /**
     * Permet d'ajouter une livraison. Ne doit servir que lors de la création de
     * la demande de livraison lors de l'ajout de l'entrepot
     * 
     * @param Livraison
     *            entrepot
     */
    ...
    /**
     * Permet d'ajouter une fenetre temporelle. Ne doit servir que lors de la
     * création de la demande de livraison
     * 
     * @param FenetreTemporelle
     *            fenetreTemporelle
     */
